...
public class ModeloJogo {
    ...
    public Item encontrarItem(boolean isExploracao) {
        Random random = new Random();
        int chanceEncontrar = isExploracao ? 75 : 85; // 75% para exploração, 85% para pós-combate
    ...
    private Item gerarItemDoDia(boolean isPosCombate) { // Adicionar parâmetro isPosCombate
        Random random = new Random();
        int tipoItem = random.nextInt(16); // Inicializar com um valor aleatório padrão

        if (isPosCombate && random.nextInt(100) < 60) { // 60% de chance de priorizar itens de cura no pós-combate
        ...
        if (nomeDoLocalPrincipal != null) { // Verifica o nome do local principal
            if (nomeDoLocalPrincipal.equals("Delegacia") || nomeDoLocalPrincipal.equals("Base Militar")) {
                ...
                if (chanceMuni < 71) { // 71% de chance de ser munição para atingir ~50% total
